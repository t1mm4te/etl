...
describe('pipelines api', () => {
  ...
  it('calls pipeline and datasource endpoints with expected payloads', async () => {
    ...
    await expect(createPipeline({ name: 'Demo', description: 'Test' })).resolves.toEqual({ id: 'pipe-1' });
    expect(apiClientMock.post).toHaveBeenCalledWith('/pipelines/', { name: 'Demo', description: 'Test' });
    ...
    expect(apiClientMock.post).toHaveBeenCalledWith('/pipelines/pipe-1/nodes/', expect.objectContaining({ label: 'Source' }));

    apiClientMock.patch.mockResolvedValueOnce({ data: { id: 'node-1' } });
    await expect(patchNode('pipe-1', 'node-1', { label: 'Updated' })).resolves.toEqual({ id: 'node-1' });
    expect(apiClientMock.patch).toHaveBeenCalledWith('/pipelines/pipe-1/nodes/node-1/', { label: 'Updated' });
    ...
    expect(apiClientMock.post).toHaveBeenCalledWith('/pipelines/pipe-1/edges/', expect.objectContaining({ source_node: 'a' }));
    ...
    await expect(uploadDatasource(uploadFile, 'Sample', 'Sheet1')).resolves.toEqual({ id: 'source-1' });
    ...
    expect(apiClientMock.get).toHaveBeenCalledWith('/datasources/source-1/preview/', { params: { limit: 5 } });

    apiClientMock.get.mockResolvedValueOnce({ data: { columns: {} } });
    await expect(previewNodeRun('node-run-1', 10)).resolves.toEqual({ columns: {} });
    expect(apiClientMock.get).toHaveBeenCalledWith('/node-runs/node-run-1/preview/', { params: { limit: 10 } });
    ...
    expect(apiClientMock.get).toHaveBeenCalledWith('/files/', { params: { limit: 10, offset: 20 } });
    ...
    expect(apiClientMock.get).toHaveBeenCalledWith('/datasources/', { params: { source_file_id: 'file-1' } });
    ...
    expect(apiClientMock.post).toHaveBeenCalledWith(
      '/files/',
      expect.any(FormData),
      expect.objectContaining({ headers: { 'Content-Type': 'multipart/form-data' }, onUploadProgress: expect.any(Function) })
    );

    apiClientMock.post.mockResolvedValueOnce({ data: { id: 'source-4' } });
    await expect(createDatasourceFromSheet('source-file-1', 'Sheet1')).resolves.toEqual({ id: 'source-4' });
